        ...
        private void DoPrisonerPropaganda()
        {
            if (this.parent.IsHashIntervalTick(Props.recruitmentRate))
            {
                foreach (var recipient in PawnsAround(Props.recruitmentRadius))
                {
                    ...
                    {
                        if (recipient.guest.ExclusiveInteractionMode == PrisonerInteractionModeDefOf.ReduceResistance || recipient.guest.ExclusiveInteractionMode == PrisonerInteractionModeDefOf.AttemptRecruit)
                        {
                            ...
                            if (resistance > 0)
                            {
                                ...
                                {
                                    recipient.guest.SetRecruitmentData(pawn);
                                }
                            }
                        }
                        else if (recipient.guest.ExclusiveInteractionMode == PrisonerInteractionModeDefOf.ReduceWill || recipient.guest.ExclusiveInteractionMode == PrisonerInteractionModeDefOf.Enslave)
                        {
